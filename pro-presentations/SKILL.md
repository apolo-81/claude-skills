...
description: >
  Creates high-impact professional presentations (keynotes, pitch decks, commercial proposals)
  using Duarte, Reynolds, Alley, Minto, and McKinsey/BCG methodology.
  Use when user requests: "pitch deck", "investor deck", "keynote", "consulting deck",
  "TED-style", "slide deck", "presentacion profesional", "presentacion impactante",
  "propuesta visual", "deck para cliente", "presentacion para auditorio", "crear slides".
  Do NOT use for: quick internal meeting notes, simple status updates, or one-slide summaries.
---
...
- **`ppt-creator`** — Use for editable PPTX export and chart generation via `chartkit.py`. This skill handles strategy, design, and content; ppt-creator handles file conversion.
- **`market-visual-identity`** — Upstream skill. If the brand's visual identity is not yet defined, run `/market visual-identity` first to generate `tokens.json`, then use those tokens as the design foundation in Step 4 instead of inventing ad-hoc colors/fonts.
...
4. **Apply Design System** — See `references/design-system.md` for typography, color, layout specs. **If a `tokens.json` file exists in the working directory (output of `market-visual-identity`), import its values directly instead of defining ad-hoc colors/fonts.**
...
8. **Package deliverables** — Save to `/output/` (slides.md, notes.md, refs.md, assets/)
...
| File | Content |
|------|---------|
| `slides.md` | Marp-compatible. Assertion headlines, content, `![alt](image)` placeholders, `<!-- notes -->` blocks. |
...
`slides.md` output is Marp-compatible. Choose the rendering path based on deliverable needs:

| Format | Command | Requirement | Notes |
|--------|---------|-------------|-------|
| Live preview | VS Code + Marp extension | VS Code | Recommended for iteration with client |
| HTML | `marp slides.md -o output.html` | Node.js 18+, Chrome/Edge | Self-contained, browser-navigable |
| PDF | `marp slides.md --pdf` | Node.js 18+, Chrome/Edge | Best for distribution and printing |
| PPTX (images) | `marp slides.md --pptx` | Node.js 18+, Chrome/Edge | Slides as rendered images — not text-editable |
| PPTX (editable) | Use **ppt-creator** skill (pptxgenjs) | Python/Node | Required when client needs to edit the deck |

**Install Marp CLI:** `npm install -g @marp-team/marp-cli`

**Rule:** Default to PDF for client delivery. Use editable PPTX only when explicitly requested — it requires the ppt-creator skill and adds generation complexity.
